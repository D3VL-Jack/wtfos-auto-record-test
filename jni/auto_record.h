 ...
 * Version: v1
 */
#include <android/log.h>  // android logging library
...
/* Debugging */
// #define DEBUG 1  // comment in to write debug logs
#undef DEBUG  // comment out to write debug logs
#ifdef DEBUG
...
/* DJI typedefs from dji_glasses */

/* Define typedefs to enums */
typedef enum record_state record_state_t;  // ptype record_state_t
...
/* DJI enums from dji_glasses */

/* au link status */
typedef enum gs_link_stat_t {  // ptype gs_link_stat_t
...
typedef enum Record_sender {  // ptype Record_sender
    RECORD_BUTTON = 0,
    RECORD_DISARM = 1
} Record_sender;

/* recording state */
typedef enum record_state {  // ptype enum record_state
    RECORD_STATE_IDLE = 0,
...
/* record mode */
typedef enum record_mode {  // ptype enum record_mode
	RECORD_MODE_MANNUAL = 0,
	RECORD_MODE_LOOPING = 1
} record_mode;


/* DJI structs from dji_glasses */

/* queue for gui items */
struct __gs_queue {  // ptype struct __gs_queue
    unsigned char *addr;
    uint32_t size;
    uint32_t num;
    uint32_t writer;
...
/* gui config - settings */
struct __gs_gui_config {  // ptype struct __gs_gui_config
	...
	int (* gs_ext_fc_get_battery_state)(void *ctx,void *state);
	...
	int (* gs_ext_fc_get_racing_osd_info)(void *ctx,void *osdInfo);
	...
	int (* gs_ext_fc_get_rtc_date)(void *ctx,void *rtc);
	...
	int (* gs_sd_get_info_wrap)(void *ctx,void *sd_info);
	int (* gs_get_battery_info)(void *ctx,void *bat_info);
	...
	int (* gs_player_get_fileinfo)(char *filename,void *file_info);
	int (* gs_player_is_playing)(void *ctx,bool *is_playing);
	int (* gs_player_delete_file)(char *filename);
	int (* gs_send_camera_cmd_async)(void *event_obj,int cam_cmd,void *msg,uint32_t msg_len,uint16_t seq_id);
	...
	int (* gs_modem_get_signal_quality)(void *ctx,void *signal_quality);
	int (* gs_modem_get_rc_signal_quality)(void *ctx,void *rc_signal_quality);
	...
	int (* gs_rc_get_subtrim_sync)(void *ctx,void *subtrim);
	int (* gs_rc_get_subtrim_async)(void *ctx);
	int (* gs_rc_set_reverse)(void *ctx,uint8_t stick,uint8_t rev);
	int (* gs_rc_get_reverse_sync)(void *ctx,void *reverse);
	...
	int (* gs_rc_set_warning_mode)(void *ctx,int warn);
	int (* gs_rc_get_warning_mode)(void *ctx,void *warn);
	int (* gs_rc_monitor_setup)(void *ctx,void *monitor);
	...
	int (* gs_vcm_register_channel_switch_callback_wrap)(void *ctx,int callback,void *data);
	int (* gs_vcm_register_channel_push_callback_wrap)(void *ctx,int callback,void *data);
	int (* gs_vcm_query_current_channel_wrap)(void *ctx,void *chnl_id);
	int (* gs_vcm_enable_channel_wrap)(void *ctx,int chnl_id);
	int (* gs_vcm_disable_channel_wrap)(void *ctx,int chnl_id);
	int (* gs_vcm_enable_sub_channel_wrap)(void *ctx,void *sub_chnl_id);
	int (* gs_vcm_disable_sub_channel_wrap)(void *ctx,void *sub_chnl_id);
	...
	int (* gs_common_send_cmd)(void *ctx,void *cmd,bool sync);
	int (* gs_get_codec_debug_osd_info)(void *ctx,void *info);
	int (* gs_get_cam_debug_osd_info)(void *ctx,void *info);
	int (* gs_get_temp_debug_osd_info)(void *ctx,void *info);
	int (* gs_get_uav_max_temp)(void *ctx,int32_t *uav_temp);
	int (* gs_get_cp_debug_osd_info)(void *ctx,void *info);
	...
	int (* gs_lv_transcode_set_looping_mode)(void *ctx,int rec_mode);
	...
	int (* gs_get_pigeon_battery_info)(void *ctx,void *vol);
...
static void *libtp1801_gui_dji = 0;  // static void (any type) pointer - default 0 - the gui library
gs_gui_config_t *ui_config = 0;  // pointer to struct - default 0
static uint32_t *hardware_info = 0;  // static unsigned integer 32 bit pointer - default 0

/* Define internal pointer functions */
static uint32_t (*libtp1801_timer)(void *this) = 0;  // static unsigned integer 32 bit pointer function with parameter empty pointer variable this - () prevents overwriting - default is zero
static uint32_t (*libtp1801_settings)() = 0;  // static unsigned integer 32 bit pointer function without paremeters - () prevents overwriting - default is zero

/* Define external pointer functions */
static int32_t (*gs_modem_get_link_state_wrap)(void *, enum gs_link_stat_t *) = 0;  // initialize pointer to function with variables and default value 0
static int32_t (*gs_lv_transcode_get_state)(void *) = 0;  // initialize pointer to function with variable and default value 0

/* Define functions */
void init();
uint32_t retrieve_timer(void *this);
uint32_t retrieve_settings();
